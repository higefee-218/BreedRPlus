...
run_gblup <- function(qc_results,
  ...
  # Convert FBM to regular matrix (bed_tcrossprodSelf returns FBM which can't use diag() directly)
  # Extract as regular matrix to allow diagonal modification - this works for both FBM and regular matrices
  G_full <- G_full[, ]
  # Modify diagonal in place to avoid "long vectors not supported" error with diag() on large matrices
  # Use linear indexing to modify diagonal without extracting it as a vector
  n_g <- nrow(G_full)
  diag_pos <- seq(1, n_g * n_g, by = n_g + 1)
  G_full[diag_pos] <- G_full[diag_pos] + tol_diag
...
cv_gblup <- function(qc_results,
  ...
  n_g <- nrow(G_full)
  diag_pos <- seq(1, n_g * n_g, by = n_g + 1)
  G_full[diag_pos] <- G_full[diag_pos] + 1e-6
